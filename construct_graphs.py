...
if __name__ == '__main__':
    train_graph = make_graph(get_filename=Data.get_train, hash_type='np_nt', replace_id_w_name=True)
    test_graph = make_graph(get_filename=Data.get_test, hash_type='np_nt', replace_id_w_name=True)
    combined_graph = make_graph(get_filename=Data.get_all, hash_type='np_nt', replace_id_w_name=True)
    ...
    exp1_graph_with_added_nodes_and_edges, added_edges1, _ = copy_over_some_edges(add_to_graph=copy.deepcopy(exp1_graph),
                                                                                             source_graph=test_graph,
                                                                                             new_nodes=nodes_to_add_all_edges,
                                                                                             copy_percent=1.0)
    exp1_graph_with_added_nodes_and_edges, added_edges2, missing_edges = copy_over_some_edges(add_to_graph=exp1_graph_with_added_nodes_and_edges,
                                                                                             source_graph=test_graph,
    ...
    base_graph_missing = (combined_graph.nodes - exp1_graph.nodes)
    for missing_node in base_graph_missing:
        exp1_graph.add_node(missing_node)
    ...
    gpt_query = f"I will give you a partially completed knowledge graph that I want you to complete. The knowledge graph connects information about patients" \
                f"with allergies, conditions, medications, and health observations that are described by SNOMED-CT codes. The graph I will give" \
    ...
    dist_edit_combo_to_gpt = edit_dist(G1=combined_graph, G2=exp1_graph_with_added_nodes_and_edges)
    dist_s_combo_to_gpt = spectral_dist(G1=combined_graph, G2=exp1_graph_with_added_nodes_and_edges, p=2, kind="laplacian")

    dist_edit_combo_to_noedge = edit_dist(G1=combined_graph, G2=exp1_graph)
    dist_s_combo_to_noedge = spectral_dist(G1=combined_graph, G2=exp1_graph, p=2, kind="laplacian")
